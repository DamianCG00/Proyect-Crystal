...
def iniciar_honeypot():
    ...
    server.listen(5)
    print("[Kalopsia] Honeypot escuchando en puerto 8080...")
    
    while True:
        conn, addr = server.accept()
        ip = addr[0]
        print(f"[Kalopsia] Intrusión detectada desde: {ip}")
        try:
            ...
            print("Error BD Kalopsia:", e)
        conn.close()

def arrancar_en_hilo():
    hilo = threading.Thread(target=iniciar_honeypot, daemon=True)
    hilo.start()